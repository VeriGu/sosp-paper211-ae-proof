...
Definition wait_hlock_body :=
  (Scall None
    (Evar wait_qlock (Tfunction (Tcons tuint Tnil) tvoid cc_default))
    ((Etempvar _lk tuint) :: nil)).

Definition f_wait_hlock := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_lk, tuint) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body := wait_hlock_body
|}.

Definition pass_hlock_body :=
  (Scall None
    (Evar pass_qlock (Tfunction (Tcons tuint Tnil) tvoid cc_default))
    ((Etempvar _lk tuint) :: nil)).

Definition f_pass_hlock := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_lk, tuint) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body := pass_hlock_body
|}.
