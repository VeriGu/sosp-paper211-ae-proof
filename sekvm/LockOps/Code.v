...
Definition _t'1 := 120%positive.
Definition _t'2 := 121%positive.
Definition _t'3 := 122%positive.

Definition wait_lock_loop_condition :=
  (Ebinop One (Etempvar _my_ticket tuint) (Etempvar _cur_ticket tuint) tint).

Definition wait_lock_loop_body :=
  (Ssequence
    (Scall (Some _t'3)
      (Evar get_now (Tfunction (Tcons tuint Tnil) tuint cc_default))
      ((Etempvar _lk tuint) :: nil))
    (Sset _cur_ticket (Etempvar _t'3 tuint))).

Definition wait_lock_body :=
(Ssequence
  (Ssequence
    (Scall (Some _t'1)
      (Evar incr_ticket (Tfunction (Tcons tuint (Tcons tuint Tnil)) tuint
                           cc_default))
      ((Econst_int (Int.repr 10) tuint) :: (Etempvar _lk tuint) :: nil))
    (Sset _my_ticket (Etempvar _t'1 tuint)))
  (Ssequence
    (Ssequence
      (Scall (Some _t'2)
        (Evar get_now (Tfunction (Tcons tuint Tnil) tuint cc_default))
        ((Etempvar _lk tuint) :: nil))
      (Sset _cur_ticket (Etempvar _t'2 tuint)))
    (Ssequence
      (Swhile
        (Ebinop One (Etempvar _my_ticket tuint) (Etempvar _cur_ticket tuint)
          tint)
        wait_lock_loop_body)
      (Ssequence
        (Scall None
          (Evar log_hold (Tfunction (Tcons tuint Tnil) tvoid cc_default))
          ((Etempvar _lk tuint) :: nil))
        (Scall None (Evar barrier (Tfunction Tnil tvoid cc_default)) nil))))).
...
Definition f_wait_lock := {|
  ...
  fn_vars := nil;
  fn_temps := ((_my_ticket, tuint) :: (_cur_ticket, tuint) ::
               (_t'3, tuint) :: (_t'2, tuint) :: (_t'1, tuint) :: nil);
  fn_body := wait_lock_body
|}.

Definition pass_lock_body :=
  (Ssequence
    (Scall None (Evar barrier (Tfunction Tnil tvoid cc_default)) nil)
    (Scall None
      (Evar incr_now (Tfunction (Tcons tuint Tnil) tvoid cc_default))
      ((Etempvar _lk tuint) :: nil))).
