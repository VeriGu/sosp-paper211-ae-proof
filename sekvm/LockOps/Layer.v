...
Section LockOpsLayer.
  ...
  Section InvDef.
    ...
    Record high_level_invariant (abd: RData) :=
      mkInvariant {
        ...
        (* valid_curid: 0 <= ZMap.get abd.(curid) abd.(cid) < num_proc *)
      }.

    Global Instance LockOps_ops : CompatDataOps RData :=
      {
  ...
  Section InvInit.
    ...
    Global Instance LockOps_prf : CompatData RData.
    Proof.
  ...
  Section InvProof.
    ...
    Global Instance wait_lock_inv: PreservesInvariants wait_lock_spec.
    Proof.
      constructor; intros; simpl; eauto.
      constructor.
    Qed.

    Global Instance pass_lock_inv: PreservesInvariants pass_lock_spec.
    Proof.
      constructor; intros; simpl; eauto.
      constructor.
    Qed.
  ...
  Section LayerDef.
    ...
    Definition LockOps_fresh : compatlayer (cdata RData) :=
      wait_lock ↦ gensem wait_lock_spec
    ⊕ pass_lock ↦ gensem pass_lock_spec
    .

    Definition LockOps_passthrough : compatlayer (cdata RData) :=
    ...
    Definition LockOps := LockOps_fresh ⊕ LockOps_passthrough.
